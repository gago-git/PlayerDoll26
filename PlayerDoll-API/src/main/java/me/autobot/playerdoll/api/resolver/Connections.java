...
public final class Connections {
    ...
    static {
        ...
        List<Field> packetListenerFields = Arrays.stream(connectionClass.getDeclaredFields())
                .filter(field -> field.getType() == C_PACKET_LISTENER)
                .toList();
        connectionPacketListenerField = packetListenerFields.stream()
                .filter(field -> field.getName().equals("packetListener"))
                .findFirst()
                .orElseGet(() -> {
                    if (packetListenerFields.size() >= 2) {
                        // Historical fallback that worked across older versions.
                        return packetListenerFields.get(1);
                    }
                    return packetListenerFields.stream().findFirst().orElseThrow();
                });
