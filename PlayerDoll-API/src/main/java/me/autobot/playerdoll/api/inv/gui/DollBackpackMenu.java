...
//    }
    ...
    public void click(InventoryClickEvent event) {
        if (event.getClickedInventory() == null) {
            onClickOutside((Player) event.getWhoClicked());
            return;
        }
        ...
        if (event.getClickedInventory() == event.getView().getBottomInventory()) {
            ...
            PlayerDollAPI.getScheduler().entityTaskDelayed(() -> {
                syncMenuToDollInventory();
                updateGUIContent();
            }, dollPlayer, 1);
            return;
        }
        if (event.getSlot() >= 5 && event.getSlot() <= 7) {
            event.setCancelled(true);
            return;
        }
        if (event.getSlot() == 8) {
            // Real backpack
            if (dollInventory.getSize() % 9 == 0) {
    ...
    private void syncMenuToDollInventory() {
        ItemStack[] items = inventory.getContents();
        dollInventory.setHelmet(items[0]);
        dollInventory.setChestplate(items[1]);
        dollInventory.setLeggings(items[2]);
        dollInventory.setBoots(items[3]);
        dollInventory.setItemInOffHand(items[4]);
        for (int i = 0; i < 9; i++) {
            dollInventory.setItem(i + 9, items[9 + i]);
            dollInventory.setItem(i + 18, items[18 + i]);
            dollInventory.setItem(i + 27, items[27 + i]);
            dollInventory.setItem(i, items[36 + i]);
        }
    }
