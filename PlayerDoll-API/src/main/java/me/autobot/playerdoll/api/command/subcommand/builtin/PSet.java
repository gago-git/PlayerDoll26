...
public class PSet extends SubCommand implements DollCommandExecutor {
    ...
    public void execute() {
        ...
        Map<PersonalFlagButton, Boolean> flagMap = config.playerSetting.get(ReflectionUtil.getGameProfileId(profile));
        if (flagMap == null) {
            ...
            config.playerSetting.put(ReflectionUtil.getGameProfileId(profile), defaultMap);

            DollMenuHolder.HOLDERS.get(target.getUniqueId()).getPSetMenu(Bukkit.getOfflinePlayer(ReflectionUtil.getGameProfileId(profile)));
            flagMap = config.playerSetting.get(ReflectionUtil.getGameProfileId(profile));
        }
        flagMap.put(flagType, toggle);

        if (flagType == PersonalFlagButton.HIDDEN) {
            Player psetPlayer = Bukkit.getPlayer(ReflectionUtil.getGameProfileId(profile));
            if (psetPlayer == null) {
                return;
            }
    ...
    private void openGUI() {
        if (sender instanceof Player player) {
            player.openInventory(DollMenuHolder.HOLDERS.get(target.getUniqueId()).getPSetMenu(Bukkit.getOfflinePlayer(ReflectionUtil.getGameProfileId(profile))).getInventory());
        }
    }
...
//        }
        ...
        if (fileUtil.getFile(fileUtil.getDollDir(), DollNameUtil.dollShortName(ReflectionUtil.getGameProfileName(profile)) + ".yml").exists()) {
