...
public class Create extends SubCommand implements DollCommandExecutor {
    ...
    public void execute() {
        ...
        if (profile != null) {
            if (!ReflectionUtil.getGameProfileProperties(profile).get("textures").isEmpty()) {
                Property property = ReflectionUtil.getGameProfileProperties(profile).get("textures").iterator().next();
            ...
            Property property = ReflectionUtil.getGameProfileProperties(fetchProfile).get("textures").iterator().next();
            dollConfig.skinProperty.setNewValue(property.value());
            dollConfig.skinSignature.setNewValue(property.signature());
        }
