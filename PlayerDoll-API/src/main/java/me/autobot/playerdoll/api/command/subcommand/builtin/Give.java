...
public class Give extends SubCommand implements DollCommandExecutor {
    ...
    public void execute() {
        if (PlayerDollAPI.getConfigLoader().getPermConfig().enable.getValue()) {
            ...
            if (!oldOwnerUUID.equals(ReflectionUtil.getGameProfileId(profile))) {
                int oldOwnerCount = creationCounts.get(oldOwnerUUID);
                Integer newOwnerCount = creationCounts.get(ReflectionUtil.getGameProfileId(profile));
                if (newOwnerCount == null) {
                    newOwnerCount = 0;
                }

                creationCounts.put(oldOwnerUUID,oldOwnerCount-1);
                creationCounts.put(ReflectionUtil.getGameProfileId(profile),newOwnerCount+1);
            }
        }

        dollConfig.ownerName.setNewValue(ReflectionUtil.getGameProfileName(profile));
        dollConfig.ownerUUID.setNewValue(ReflectionUtil.getGameProfileId(profile).toString());
...
//        }
        ...
        Player newOwner = Bukkit.getPlayer(ReflectionUtil.getGameProfileId(profile));
        if (newOwner == null) {
            sender.sendMessage(LangFormatter.YAMLReplaceMessage("player-offline"));
            return 0;
        }

        if (targetString.equalsIgnoreCase(ReflectionUtil.getGameProfileName(profile))) {
