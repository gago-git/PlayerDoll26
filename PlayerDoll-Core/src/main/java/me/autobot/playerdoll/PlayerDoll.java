...
public final class PlayerDoll extends JavaPlugin implements PlayerDollPlugin {
    ...
    public void onEnable() {
        ...
        // Update checker intentionally disabled in this branch.
        checkUpdate();
    ...
    public void onDisable() {
        ...
        DollConfig.DOLL_CONFIGS.forEach((uuid, dollConfig) -> {
            Doll doll = DollStorage.ONLINE_DOLLS.get(uuid);
            if (doll != null) {
                dollConfig.setLastLocation(doll.getBukkitPlayer().getLocation());
            }
...
//        }
    ...
    private void checkUpdate() {
        // No-op: remote version comparator disabled.
    }
        ...
        for (File dollFile : dollConfigs) {
            ...
            DollStorage.PLAYER_CREATION_COUNTS.merge(ownerUUID, 1, Integer::sum);
                ...
                if (config.dollSetting.get(GlobalFlagButton.JOIN_AT_START).getValue()) {
                    GameProfile profile = ReflectionUtil.createGameProfileWithTextures(
                            UUID.fromString(config.dollUUID.getValue()),
                            dollIdentifier.concat(config.dollName.getValue()),
                            config.skinProperty.getValue(),
                            config.skinSignature.getValue()
                    );
