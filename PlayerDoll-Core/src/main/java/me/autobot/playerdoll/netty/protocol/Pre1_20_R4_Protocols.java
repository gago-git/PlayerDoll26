...
public abstract class Pre1_20_R4_Protocols {
    ...
    static {
        ...
        for (Enum<?> protocol : connectionProtocolEnums) {
            ...
            switch (name) {
                case "login" -> {
                    CLIENT_LOGIN_PROTOCOL = ReflectionUtil.invokeMethod(connectionProtocolCodecMethod, protocol, Connections.packetFlow_Clientbound);
                    SERVER_LOGIN_PROTOCOL = ReflectionUtil.invokeMethod(connectionProtocolCodecMethod, protocol, Connections.packetFlow_Serverbound);
                }
                case "configuration" -> {
                    CLIENT_CONFIGURATION_PROTOCOL = ReflectionUtil.invokeMethod(connectionProtocolCodecMethod, protocol, Connections.packetFlow_Clientbound);
                    SERVER_CONFIGURATION_PROTOCOL = ReflectionUtil.invokeMethod(connectionProtocolCodecMethod, protocol, Connections.packetFlow_Serverbound);
                }
                case "play" -> {
                    CLIENT_PLAY_PROTOCOL = ReflectionUtil.invokeMethod(connectionProtocolCodecMethod, protocol, Connections.packetFlow_Clientbound);
                    SERVER_PLAY_PROTOCOL = ReflectionUtil.invokeMethod(connectionProtocolCodecMethod, protocol, Connections.packetFlow_Serverbound);
                }
            }
        }
