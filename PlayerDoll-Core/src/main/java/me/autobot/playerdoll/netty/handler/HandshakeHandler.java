...
public class HandshakeHandler extends ChannelDuplexHandler {
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        DollConnection.DOLL_CONNECTIONS.put(ReflectionUtil.getGameProfileId(profile), ctx.channel());
        PlayerDollAPI.getLogger().log(Level.INFO, "Doll client {0} started", ReflectionUtil.getGameProfileName(profile));
        ...
        while (!ConnectionFetcher.startCursedConnection(ctx.channel().localAddress().toString(), profile, caller)) {
            count++;
            if (count >= 10) {
                PlayerDollAPI.getLogger().info(String.format("Client %s wait too Long", ReflectionUtil.getGameProfileName(profile)));
                ctx.close();
                return;
            }
            try {
                PlayerDollAPI.getLogger().info(String.format("%s Wait for Login Listener", ReflectionUtil.getGameProfileName(profile)));
                Thread.sleep(200);
        ...
        PacketVarIntHelper.writeVarInt(loginHelloBuf, ReflectionUtil.getGameProfileName(profile).length());
        loginHelloBuf.writeCharSequence(ReflectionUtil.getGameProfileName(profile), StandardCharsets.UTF_8);
        loginHelloBuf.writeLong(ReflectionUtil.getGameProfileId(profile).getMostSignificantBits());
        loginHelloBuf.writeLong(ReflectionUtil.getGameProfileId(profile).getLeastSignificantBits());
    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        DollConnection.DOLL_CONNECTIONS.remove(ReflectionUtil.getGameProfileId(profile));
        PlayerDollAPI.getLogger().log(Level.INFO, "Doll client {0} ended", ReflectionUtil.getGameProfileName(profile));
