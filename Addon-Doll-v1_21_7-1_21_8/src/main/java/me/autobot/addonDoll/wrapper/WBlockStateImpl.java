...
public class WBlockStateImpl extends WBlockState<BlockState> {
    ...
    public float getDestroyProgress(AbsPackPlayer player, WBlockPos<?> pos) {
        return blockState.getDestroyProgress(((PackPlayerImpl)player).getServerPlayer(), ((PackPlayerImpl)player).getServerPlayer().level(), (BlockPos) pos.getInstance());
    }

    @Override
    public void attack(WBlockPos<?> pos, AbsPackPlayer packPlayer) {
        blockState.attack(((PackPlayerImpl)packPlayer).getServerPlayer().level(), (BlockPos) pos.getInstance(), ((PackPlayerImpl)packPlayer).getServerPlayer());
    }
