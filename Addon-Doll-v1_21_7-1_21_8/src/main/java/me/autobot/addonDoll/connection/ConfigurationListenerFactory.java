...
final class ConfigurationListenerFactory {
    ...
    private static final Field savedPlayerForLoginEventLegacyField;
    ...
    static {
        ...
        savedPlayerForLoginEventLegacyField = Arrays.stream(Connection.class.getDeclaredFields())
                .filter(field -> field.getType() == ServerPlayer.class)
                .filter(field -> field.getName().equals("savedPlayerForLoginEventLegacy"))
                .findFirst()
                .orElse(null);
        if (savedPlayerForLoginEventLegacyField != null) {
            savedPlayerForLoginEventLegacyField.setAccessible(true);
        }
    }
    ...
    static void setLegacySavedPlayer(Connection connection, ServerPlayer player) {
        ...
        try {
            savedPlayerForLoginEventLegacyField.set(connection, player);
        } catch (IllegalAccessException e) {
            throw new IllegalStateException("Cannot set savedPlayerForLoginEventLegacy", e);
        }
    }
}
